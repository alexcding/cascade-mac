...
@MainActor final class NativeWorkflowTerminal: WorkflowTerminal {
    ...
    init(terminal: TerminalSession, cli: WorkflowCLI, sessionID: String?) async throws {
        ...
        guard !foreground.atShell, let pgid = foreground.pgid, pgid > 0 else {
            throw BackendError.operation(String(localized: "The agent is not running, or the PTY helper needs updating. Open the terminal and check its startup before retrying."))
        }
    ...
    private func validateOwner() async throws {
        guard terminal.ready, terminal.surfaceGeneration == surfaceGeneration,
              terminal.agentTurns.streamAvailable else { throw BackendError.operation(String(localized: "The terminal or agent event connection changed during the workflow.")) }
        let current = try await terminal.workflowForeground()
        guard current == foreground else { throw BackendError.operation(String(localized: "The foreground program changed. Workflow input was stopped.")) }
    ...
    func validate(after revision: UInt64) async throws {
        ...
        guard !terminal.agentBusy, terminal.agentTurns.revision == revision else {
            throw BackendError.operation(String(localized: "Another agent turn started. Workflow input was stopped."))
        }
    }
    static func paste(_ command: String) throws -> String {
        guard !command.isEmpty, command.utf8.count <= 64 * 1024,
            ...
            throw BackendError.operation(String(localized: "Workflow commands must contain text without terminal control characters and fit within 64 KiB after expansion."))
        }
