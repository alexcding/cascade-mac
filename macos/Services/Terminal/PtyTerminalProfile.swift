...
struct PtyTerminalProfile: Codable, Sendable {
    ...
    func validate() throws {
        guard !version.isEmpty, version.utf8.count <= 128,
            ...
            throw PtyError.connection(String(localized: "The terminal creation profile is invalid."))
        }
        if let resourcesDirectory, !resourcesDirectory.hasPrefix("/") || resourcesDirectory.contains("\0") {
            throw PtyError.connection(String(localized: "The terminal shell-resource profile is invalid."))
        }
    }
    ...
    static func current() throws -> Self {
        guard let version = InMemoryTerminalSession.runtimeVersion,
            ...
            throw PtyError.connection(String(localized: "The native terminal version or bundled terminfo is missing. Rebuild the app before creating a terminal."))
        }
