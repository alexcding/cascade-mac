...
# libsqlite3-sys → rusqlite → cascade-backend recompiling on nearly every run
...
# dirs ahead of the login PATH, and cascade-vt/build.rs compiles its shim with a
...
CASCADE_CARGO_PATH="$HOME/.cargo/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin:/usr/local/bin"

cargo_build() {
  env -i \
    PATH="$CASCADE_CARGO_PATH" HOME="$HOME" \
