...
cargo_build build --locked --release --manifest-path "$ROOT/crates/cascade-backend/Cargo.toml"
cargo_build build --locked --release --manifest-path "$ROOT/crates/cascade-ptyd/Cargo.toml" --features terminal-snapshots
...
rm -rf "$APP/Contents/Resources/CascadeImages" "$APP/Contents/Resources/backend"
mkdir -p "$APP/Contents/Resources/CascadeImages"
...
test -f "$GHOSTTY_PKG/LICENSE-ghostty" || { echo "GhosttyTerminal checkout not found at $GHOSTTY_PKG; set CASCADE_GHOSTTY_PACKAGE" >&2; exit 1; }
...
codesign --force --sign - "$APP/Contents/Helpers/cascade-ptyd"
