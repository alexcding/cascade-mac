...
fn native_bytes_and_legacy_text_coexist_on_the_same_terminal() {
    let directory = std::path::PathBuf::from(format!("/tmp/cascade-ptyd-bytes-{}", std::process::id()));
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_cascade-ptyd"))
        .arg(&directory).env("CASCADE_PTYD_SOCK", &socket)
...
fn standalone_helper_preserves_protocol_and_reconnects() {
    let directory = std::path::PathBuf::from(format!("/tmp/cascade-ptyd-test-{}", std::process::id()));
    std::fs::create_dir_all(&directory).unwrap();
    let socket = directory.join("ptyd.sock");
    let child = Command::new(env!("CARGO_BIN_EXE_cascade-ptyd"))
        .arg(&directory).env("CASCADE_PTYD_SOCK", &socket)
    ...
    assert_eq!(response["ok"]["protocol"], cascade_ptyd::PROTOCOL);
...
fn startup_command_runs_after_zsh_startup_and_leaves_an_interactive_shell() {
    let directory = std::path::PathBuf::from(format!("/tmp/cascade-ptyd-startup-{}", std::process::id()));
    std::fs::create_dir_all(&directory).unwrap();
    // The startup file runs before the command; the command sees what it set.
    std::fs::write(directory.join(".zshrc"), "export CASCADE_STARTUP_ORDER=loaded\n").unwrap();
    let socket = directory.join("ptyd.sock");
    let child = Command::new(env!("CARGO_BIN_EXE_cascade-ptyd"))
        .arg(&directory).env("CASCADE_PTYD_SOCK", &socket)
