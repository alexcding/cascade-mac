...
impl Connection {
    ...
    fn hello(&mut self) {
        let hello = self
            ...
            "snapshotRevision":cascade_vt::GHOSTTY_REVISION}))
            .unwrap();
        assert_eq!(hello["snapshotRevision"], cascade_vt::GHOSTTY_REVISION);
    }
...
impl Fixture {
    fn start() -> Self {
        ...
        let child = Command::new(env!("CARGO_BIN_EXE_cascade-ptyd"))
            .arg(&root)
            .env("CASCADE_PTYD_SOCK", &socket)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
...
"#).unwrap();
    ...
    assert_eq!(hello["geometryResponseOwner"], cascade_vt::GEOMETRY_RESPONSE_OWNER);
    let mut opts = json!({"cwd":fixture.root,"shell":script,
        "stateResponseOwner":cascade_vt::IDENTITY_RESPONSE_OWNER,
        "terminalProfile":{"version":"test","terminfoDirectory":terminfo},
        "geometryResponseOwner":cascade_vt::GEOMETRY_RESPONSE_OWNER,"geometry":initial});
    for (field, value) in [
        ("geometryResponseOwner", json!("future-owner")),
        ("geometryResponseOwner", Value::Null),
        ("stateResponseOwner", json!(cascade_vt::STATE_RESPONSE_OWNER)),
        ("geometry", Value::Null),
    ...
    assert_eq!(term["geometryResponseOwner"], cascade_vt::GEOMETRY_RESPONSE_OWNER);
    drop(connection);
    ...
    let mut restored = cascade_vt::Terminal::restore(&bytes).unwrap();
    assert_eq!(restored.geometry().unwrap(), cascade_vt::Geometry { cols:80, rows:24, cell_width:9, cell_height:18 });
    ...
    restored = cascade_vt::Terminal::restore(&bytes).unwrap();
    ...
    assert_eq!(list[0]["geometryResponseOwner"], cascade_vt::GEOMETRY_RESPONSE_OWNER);
}
...
"#,
    ...
    let mut current = cascade_vt::Terminal::restore(&new_bytes).unwrap();
...
"#,
    ...
    assert_eq!(
        ...
        cascade_vt::STATE_RESPONSE_OWNER
    );
    assert!(connection
        .request(json!({"op":"create", "opts":{"stateResponseOwner":"future-owner"}}))
        .is_err());
    for opts in [
        json!({"stateResponseOwner":cascade_vt::IDENTITY_RESPONSE_OWNER}),
        json!({"terminalProfile":{"version":"1.0","terminfoDirectory":"/tmp"}}),
        json!({"stateResponseOwner":cascade_vt::IDENTITY_RESPONSE_OWNER,"terminalProfile":{"version":"bad\u{1b}version","terminfoDirectory":"/tmp"}}),
        json!({"stateResponseOwner":cascade_vt::IDENTITY_RESPONSE_OWNER,"terminalProfile":{"version":"1.0","terminfoDirectory":"relative"}}),
    ] {
        assert!(connection
    ...
    let term = connection.request(json!({"op":"create", "opts":{
        "cwd":fixture.root, "shell":script, "stateResponseOwner":cascade_vt::STATE_RESPONSE_OWNER
    }})).unwrap();
    let id = term["id"].as_str().unwrap();
    assert_eq!(term["stateResponseOwner"], cascade_vt::STATE_RESPONSE_OWNER);
    drop(connection);
