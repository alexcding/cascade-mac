// ptyd — the detached PTY daemon. Every terminal Cascade opens lives HERE, not in the app process,
// so quitting, crashing, or rebuilding Cascade never kills a shell (the unpeel / tmux model: the app
// is only an attachment). `cascade __ptyd__ <dir>` runs this loop outside the app, so terminals
// outlive it; the
...
//   /tmp/cascade-ptyd-<uid>.sock  the control socket (see sock_path)
...
// /tmp/cascade-<uid>/ is created 0700. Callers verify the path's owner before trusting it.
pub fn sock_path() -> PathBuf {
  // CASCADE_PTYD_SOCK overrides it — for tests and for running a second, isolated daemon.
  if let Some(p) = std::env::var_os("CASCADE_PTYD_SOCK").filter(|p| !p.is_empty()) {
  ...
  let dir = std::env::var_os("TMPDIR")
    ...
    .unwrap_or_else(|| {
      let d = PathBuf::from(format!("/tmp/cascade-{uid}"));
      let _ = std::fs::DirBuilder::new().mode(0o700).create(&d);
      d
    });
  dir.join("cascade-ptyd.sock")
}
...
struct Ring {
  ...
  #[cfg(feature = "terminal-snapshots")]
  terminal: Result<cascade_vt::Terminal, String>,
...
impl Ring {
  fn new(geometry: Option<TerminalGeometry>) -> Result<Self, String> {
    ...
    #[cfg(feature = "terminal-snapshots")]
    let terminal = {
      let mut terminal = cascade_vt::Terminal::new(cols, rows).map_err(|e| e.to_string())?;
...
impl Daemon {
  ...
  fn create(self: &Arc<Self>, opts: CreateOpts, creator: Option<u64>) -> Result<TermInfo, String> {
    if let Some(owner) = opts.state_response_owner.as_deref() {
      #[cfg(feature = "terminal-snapshots")]
      let supported = [cascade_vt::STATE_RESPONSE_OWNER, cascade_vt::IDENTITY_RESPONSE_OWNER].contains(&owner);
      #[cfg(not(feature = "terminal-snapshots"))]
    ...
    if let Some(owner) = opts.geometry_response_owner.as_deref() {
      #[cfg(feature = "terminal-snapshots")]
      let supported = owner == cascade_vt::GEOMETRY_RESPONSE_OWNER;
      #[cfg(not(feature = "terminal-snapshots"))]
      let supported = { let _ = owner; false };
    ...
    if let Some(owner) = opts.appearance_response_owner.as_deref() {
      #[cfg(feature = "terminal-snapshots")]
      let supported = owner == cascade_vt::APPEARANCE_RESPONSE_OWNER;
      #[cfg(not(feature = "terminal-snapshots"))]
      let supported = { let _ = owner; false };
    ...
    match op {
      "hello" => {
        ...
        #[cfg(feature = "terminal-snapshots")]
        {
          ...
          hello["snapshotRevision"] = json!(cascade_vt::GHOSTTY_REVISION);
          hello["stateResponseOwner"] = json!(cascade_vt::STATE_RESPONSE_OWNER);
          hello["identityResponseOwner"] = json!(cascade_vt::IDENTITY_RESPONSE_OWNER);
          hello["geometryResponseOwner"] = json!(cascade_vt::GEOMETRY_RESPONSE_OWNER);
          hello["appearanceResponseOwner"] = json!(cascade_vt::APPEARANCE_RESPONSE_OWNER);
          hello["shellIntegration"] = json!(true);
          if let Some(revision) = req.get("snapshotRevision") {
            if !client.byte_transport || revision.as_str() != Some(cascade_vt::GHOSTTY_REVISION) {
...
// the system browser. This helper hands the URL to the app with the shell's CASCADE_RUN_ID instead,
...
fn install_browser(dir: &Path, sock: &Path) -> Option<PathBuf> {
  ...
  let script = r#"#!/bin/sh
# Installed by cascade-ptyd as BROWSER: opens a web URL in Cascade beside the terminal that asked.
case "$1" in
  http://*|https://*)
    port=$(cat PORT_FILE 2>/dev/null)
    if [ -n "$port" ] && [ -n "$CASCADE_RUN_ID" ] &&
      /usr/bin/curl -fs -m 2 -o /dev/null -G -X POST --data-urlencode "url=$1" --data-urlencode "runId=$CASCADE_RUN_ID" \
...
// Entry point for `cascade __ptyd__ <dir>`. Never returns.
