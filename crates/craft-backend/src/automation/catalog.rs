...
static CATALOG: LazyLock<Value> = LazyLock::new(|| {
    let jql = with(
        param("jql", "JQL", "jql"),
        json!({"placeholder":"project = CRAFT AND assignee = currentUser()"}),
    );
    let triggers = vec![
    ...
    let filters = vec![
        ...
        node("filter", "jira.project", "Jira", "Ticket project", "Keeps only tickets in these Jira projects.", "any", vec![with(param("keys", "Project keys", "list"), json!({"placeholder":"CRAFT, IOS"}))]),
    ...
    let actions = vec![
        ...
        node(
            ...
            "any",
            vec![with(param("script", "Script", "script"), json!({"placeholder":"echo \"$CRAFT_PR_URL\" >> ~/merged.txt"}))],
        ),
        node(
            "action",
            "craft.webhook",
    ...
    json!([
        ...
        template(
            ...
            vec![
                ...
                action("craft.notify", json!({"title":"Changes requested","body":"{{repo}}#{{pr.number}} {{pr.title}}"})),
            ],
        ),
        template(
            "dependabot",
        ...
        template(
            ...
            &["pr.review_requested"],
            json!({}),
            vec![action("craft.notify", json!({"title":"Review requested","body":"{{pr.author}}: {{pr.title}}"}))],
        ),
        template(
            "stale",
            "Stale PR → nudge",
            ...
            vec![
                filter("pr.author", json!({"mode":"in","users":["@me"]})),
                action("craft.notify", json!({"title":"Stale PR","body":"{{pr.title}} has had no update in 5 days"})),
            ],
        ),
        template(
            "ticket-assigned",
            ...
            vec![action("craft.notify", json!({"title":"Assigned: {{jira.key}}","body":"{{jira.summary}}"}))],
        ),
    ])
}
