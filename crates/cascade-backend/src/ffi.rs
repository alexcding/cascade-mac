...
/// Receives one response per `cascade_backend_request`, on a runtime thread.
...
fn start(data_dir: PathBuf, packaged: bool, instance_id: Option<String>) -> anyhow::Result<CascadeBackend> {
    use anyhow::Context;
...
/// `cascade_string_free`). `instance_id` may be null.
///
/// # Safety
/// `data_dir` and `instance_id` are NUL-terminated strings; `out` and `error`
/// are writable or null.
#[no_mangle]
pub unsafe extern "C" fn cascade_backend_start(
    data_dir: *const c_char,
    packaged: i32,
    instance_id: *const c_char,
    out: *mut *mut CascadeBackend,
    error: *mut *mut c_char,
) -> i32 {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<CascadeBackend, String> {
...
///
/// # Safety
/// `backend` came from `cascade_backend_start` and has not been stopped.
#[no_mangle]
pub unsafe extern "C" fn cascade_backend_port(backend: *const CascadeBackend) -> u16 {
...
#[no_mangle]
pub unsafe extern "C" fn cascade_backend_request(
    backend: *const CascadeBackend,
    method: *const c_char,
...
/// `cascade_backend_unsubscribe`, at `cascade_backend_stop`, or if the event
/// channel closes.
///
/// # Safety
/// `ctx` stays valid until `dropped` has run.
#[no_mangle]
pub unsafe extern "C" fn cascade_backend_subscribe(
    backend: *const CascadeBackend,
    ctx: *mut c_void,
...
/// `backend` came from `cascade_backend_start` and has not been stopped.
#[no_mangle]
pub unsafe extern "C" fn cascade_backend_unsubscribe(backend: *const CascadeBackend, id: u64) {
    if backend.is_null() {
        return;
    }
...
///
/// # Safety
/// `backend` came from `cascade_backend_start` and is stopped exactly once.
#[no_mangle]
pub unsafe extern "C" fn cascade_backend_stop(backend: *mut CascadeBackend) {
    if backend.is_null() {
        return;
    }
    ...
    let CascadeBackend { runtime, .. } = *backend;
...
#[no_mangle]
pub unsafe extern "C" fn cascade_string_free(value: *mut c_char) {
    if !value.is_null() {
...
mod tests {
    ...
    fn call(backend: *const CascadeBackend, method: &str, target: &str, body: &str) -> Reply {
        let (sender, receiver) = mpsc::channel();
        let sender = Box::new(sender);
        let method = CString::new(method).unwrap();
        let target = CString::new(target).unwrap();
        unsafe {
            cascade_backend_request(
                backend,
                method.as_ptr(),
                target.as_ptr(),
                body.as_ptr(),
    ...
    fn start_temp() -> (*mut CascadeBackend, tempfile::TempDir) {
        let dir = tempfile::tempdir().unwrap();
        let path = CString::new(dir.path().to_str().unwrap()).unwrap();
        let instance = CString::new("test-instance").unwrap();
        let mut handle: *mut CascadeBackend = std::ptr::null_mut();
        let mut error: *mut c_char = std::ptr::null_mut();
        let code = unsafe { cascade_backend_start(path.as_ptr(), 0, instance.as_ptr(), &mut handle, &mut error) };
    ...
    fn dispatches_requests_in_process_and_serves_a_loopback_port() {
        ...
        assert!(over_tcp.unwrap().contains("\"service\":\"cascade\""));
        ...
        unsafe {
            cascade_backend_request(backend, method.as_ptr(), target.as_ptr(), std::ptr::null(), 0,
                                    &*sender as *const _ as *mut c_void, Some(on_response));
            cascade_backend_stop(backend);
        }
    ...
    fn subscriptions_deliver_broadcasts_and_release_their_context() {
        ...
        let ctx = Box::into_raw(Box::new(sender)) as *mut c_void;
        let drops = DROPS.load(Ordering::SeqCst);
        assert!(unsafe { cascade_backend_subscribe(backend, ctx, Some(on_event), Some(on_dropped)) } > 0);
    ...
    fn a_terminal_browser_request_is_relayed_as_the_terminal_that_asked() {
        ...
        let id = unsafe { cascade_backend_subscribe(backend, ctx, Some(on_event), Some(on_dropped)) };
        std::thread::sleep(Duration::from_millis(50));
        // As cascade-ptyd's helper sends it: curl's form encoding, `+` for a space.
    ...
    fn the_swift_header_declares_every_exported_function() {
        ...
        let header = include_str!("../../../macos/Vendor/CascadeBackend/cascade_backend.h");
        ...
        for name in exported {
            assert!(header.contains(&format!(" {name}(")), "{name} is missing from cascade_backend.h");
        }
    }
    ...
    fn start_reports_errors_instead_of_panicking() {
        ...
        let mut handle: *mut CascadeBackend = std::ptr::null_mut();
        let mut error: *mut c_char = std::ptr::null_mut();
        let code = unsafe { cascade_backend_start(path.as_ptr(), 0, std::ptr::null(), &mut handle, &mut error) };
        assert_eq!(code, 1);
        assert!(handle.is_null());
        assert!(!error.is_null());
        assert!(unsafe { CStr::from_ptr(error) }.to_string_lossy().contains("create data directory"));
        unsafe { cascade_string_free(error) };
    }
}
