...
pub fn prime_shell_environment() {
    ...
    if cfg!(test) || std::env::var_os("CASCADE_NO_SHELL_PROBE").is_some() {
        return;
    }
    STARTED.call_once(|| {
...
const SOCKET_PROBE: &str =
    "printf '\\n<cascade-path>%s</cascade-path>\\n<cascade-ssh>%s</cascade-ssh>\\n' \"$PATH\" \"$SSH_AUTH_SOCK\"";
const PROBE_END: &str = "</cascade-ssh>";
...
fn path_from_probe(output: &str) -> Option<String> {
    let start = output.rfind("<cascade-path>")? + "<cascade-path>".len();
    let end = output[start..].find("</cascade-path>")? + start;
...
fn socket_from_probe(output: &str) -> Option<String> {
    let start = output.rfind("<cascade-ssh>")? + "<cascade-ssh>".len();
...
mod tests {
    ...
    fn socket_probe_reads_the_marked_value_past_rc_noise() {
        assert_eq!(
            socket_from_probe("Last login: today\nmotd\n<cascade-ssh>/tmp/agent.sock</cascade-ssh>\n"),
            Some("/tmp/agent.sock".to_owned())
        );
        assert_eq!(socket_from_probe("<cascade-ssh></cascade-ssh>\n"), None);
        assert_eq!(
            socket_from_probe("<cascade-ssh>relative.sock</cascade-ssh>"),
            None
        );
    ...
    fn shell_socket_is_exported_only_when_it_is_new_and_live() {
        let probe = "<cascade-ssh>/tmp/agent.sock</cascade-ssh>";
        assert_eq!(
    ...
    fn brief_run_returns_at_the_marker_while_a_grandchild_holds_the_pipe() {
        ...
        let output = run_briefly(
            ...
            &[
                "-c",
                "(sleep 30 &); printf '<cascade-ssh>/x</cascade-ssh>\\n'; sleep 30",
            ],
            Duration::from_secs(5),
        )
        .expect("marker read");
        assert!(String::from_utf8_lossy(&output).contains("</cascade-ssh>"));
        assert!(
    ...
    fn brief_run_finds_a_marker_split_across_chunks_and_caps_runaway_output() {
        ...
        let output = run_briefly(
            ...
            &[
                "-c",
                "head -c 5000 /dev/zero | tr '\\0' x; printf '<cascade-ssh>/y</cascade-ssh>\\n'",
            ],
            Duration::from_secs(5),
        )
        .expect("marker read");
    ...
    fn the_login_shell_path_is_read_past_rc_noise_and_keeps_absolute_entries() {
        let output = "motd\n<cascade-path>/Users/me/.nvm/versions/node/v22.1.0/bin:bin:/usr/bin</cascade-path>\n<cascade-ssh></cascade-ssh>\n";
        assert_eq!(
            path_from_probe(output).as_deref(),
            Some("/Users/me/.nvm/versions/node/v22.1.0/bin:/usr/bin")
        );
        assert_eq!(path_from_probe("<cascade-path></cascade-path>"), None);
        assert_eq!(path_from_probe("no marker"), None);
        // The socket still reads from the same output.
        assert_eq!(
            socket_from_probe("<cascade-path>/usr/bin</cascade-path>\n<cascade-ssh>/tmp/a.sock</cascade-ssh>\n").as_deref(),
            Some("/tmp/a.sock")
        );
    }
    ...
    async fn a_command_resolves_programs_against_the_search_path() {
        ...
        let tool = dir.path().join("cascade-path-probe");
        std::fs::write(&tool, "#!/bin/sh\necho found\n").unwrap();
        std::fs::set_permissions(&tool, std::fs::Permissions::from_mode(0o755)).unwrap();
        let path = format!("{}:/usr/bin:/bin", dir.path().display());

        assert_eq!(resolve("cascade-path-probe", &path), tool.as_os_str());
        // A path of its own is already spawnable, and may be relative to the CHILD's cwd.
        assert_eq!(resolve("./cascade-path-probe", &path), "./cascade-path-probe");
        ...
        std::fs::create_dir(dir.path().join("cascade-path-dir")).unwrap();
        let miss = resolve("cascade-path-dir", &path);
        assert_ne!(miss, "cascade-path-dir");
        assert!(Path::new(&miss).is_absolute(), "{miss:?}");
        // Nothing on PATH matches at all: still a path, and still where it would have been.
        let missing = resolve("cascade-path-probe", "/usr/bin:/bin");
        assert_eq!(missing, "/usr/bin/cascade-path-probe");
        // A file nobody may execute is not a match, and does not shadow the one further along.
        let blocked = tempfile::tempdir().unwrap();
        let shadow = blocked.path().join("cascade-path-probe");
        ...
        for program in ["sh", "cascade-definitely-not-installed"] {
        ...
        let output = Command::new(resolve("cascade-path-probe", &path))
            .output()
            .await
            .unwrap();
