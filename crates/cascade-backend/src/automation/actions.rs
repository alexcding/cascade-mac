...
pub fn validate(step: &Step) -> Result<()> {
    ...
    match step.node.as_str() {
        ...
        "cascade.shell" => need("script", "Run shell script: the script"),
        "cascade.webhook" => {
...
mod tests {
    ...
    fn describe_quotes_only_what_needs_it() {
        ...
        let plan = Plan::Transition { key: "CASCADE-1".into(), status: "In Review".into() };
        assert_eq!(plan.describe(), "acli jira workitem transition --key CASCADE-1 --status 'In Review' --yes");
    }
    ...
    fn required_params_are_checked_at_save() {
        ...
        assert!(validate(&step("cascade.webhook", json!({"url":"http://x"}))).is_err());
