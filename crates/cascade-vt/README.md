...
cargo test --manifest-path crates/cascade-vt/Cargo.toml
```
...
library. It does not install global tools. `CASCADE_GHOSTTY_VT_DIR` may select an
...
must match. Cascade's version 2 adds a checksummed glyph registration record,
...
See [the daemon snapshot protocol](../cascade-ptyd/SNAPSHOTS.md).
