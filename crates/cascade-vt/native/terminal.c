...
void *cascade_vt_new(uint16_t cols, uint16_t rows) {
...
void cascade_vt_free(void *terminal) { ghostty_terminal_free(terminal); }
void cascade_vt_feed(void *terminal, const uint8_t *bytes, size_t len) {
...
static bool cascade_color_scheme(GhosttyTerminal terminal, void *userdata, GhosttyColorScheme *out) {
    (void)terminal;
    int value = ((CascadeResponseSink *)userdata)->color_scheme;
    if (value < 0 || value > 1) return false;
    *out = (GhosttyColorScheme)value;
    return true;
}
static GhosttyString cascade_version(GhosttyTerminal terminal, void *userdata) {
    (void)terminal;
    return ((CascadeResponseSink *)userdata)->version;
}
static bool cascade_device_attributes(GhosttyTerminal terminal, void *userdata,
...
static void cascade_write_response(GhosttyTerminal terminal, void *userdata,
                                  const uint8_t *bytes, size_t len) {
    (void)terminal;
    CascadeResponseSink *sink = userdata;
...
int cascade_vt_geometry(void *terminal, uint16_t *cols, uint16_t *rows,
...
static bool cascade_size(GhosttyTerminal terminal, void *userdata,
                         GhosttySizeReportSize *out) {
    if (cascade_vt_geometry(terminal, &out->columns, &out->rows,
                           &out->cell_width, &out->cell_height) == GHOSTTY_SUCCESS) return true;
    ((CascadeResponseSink *)userdata)->failed = true;
    return false;
}
...
int cascade_vt_feed_with_responses(void *terminal, const uint8_t *bytes, size_t len,
                                 GhosttyWriterFn write, void *userdata,
                                 const uint8_t *version, size_t version_len, bool geometry, int color_scheme) {
    CascadeResponseSink sink = { .write = write, .userdata = userdata, .failed = false,
    ...
    if (result == GHOSTTY_SUCCESS) {
        result = ghostty_terminal_set(terminal, GHOSTTY_TERMINAL_OPT_WRITE_PTY,
                                     (const void *)cascade_write_response);
    }
...
int cascade_vt_resize(void *terminal, uint16_t cols, uint16_t rows) {
    return ghostty_terminal_resize(terminal, cols, rows, 0, 0);
}
int cascade_vt_resize_geometry(void *terminal, uint16_t cols, uint16_t rows,
                               uint32_t cell_width, uint32_t cell_height,
                               GhosttyWriterFn write, void *userdata) {
    uint16_t old_cols = 0, old_rows = 0;
    uint32_t old_width = 0, old_height = 0;
    if (cascade_vt_geometry(terminal, &old_cols, &old_rows, &old_width, &old_height) == GHOSTTY_SUCCESS &&
        old_cols == cols && old_rows == rows && old_width == cell_width && old_height == cell_height)
        return GHOSTTY_SUCCESS;

    CascadeResponseSink sink = { .write = write, .userdata = userdata };
    int result = ghostty_terminal_set(terminal, GHOSTTY_TERMINAL_OPT_USERDATA, &sink);
    if (result == GHOSTTY_SUCCESS)
        result = ghostty_terminal_set(terminal, GHOSTTY_TERMINAL_OPT_WRITE_PTY,
                                     (const void *)cascade_write_response);
...
int cascade_vt_snapshot(void *terminal, GhosttyWriterFn write, void *userdata) {
    return ghostty_snapshot_encode(terminal, (GhosttyWriter){ .write = write, .userdata = userdata });
}
void *cascade_vt_restore(const uint8_t *bytes, size_t len) {
...
int cascade_vt_format(void *terminal, GhosttyWriterFn write, void *userdata) {
...
int cascade_vt_cursor(void *terminal, uint16_t *x, uint16_t *y) {
...
int cascade_vt_mode(void *terminal, uint16_t number, bool ansi, bool *value) {
...
int cascade_vt_set_appearance(void *terminal, const uint32_t *values, bool notify,
                               GhosttyWriterFn write, void *userdata) {
