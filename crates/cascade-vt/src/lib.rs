...
//! The renderer and daemon share the pinned runtime and Cascade snapshot format.
//! Snapshot v3 retains glyph registrations, Kitty images, placements and transfers.
use std::{ffi::c_void, fmt, ptr::NonNull};

pub const GHOSTTY_REVISION: &str = "3c47ca159368eb4a860ffe5333abdf4a85b2767b-cascade-appearance-v3";
...
extern "C" {
    fn cascade_vt_new(cols: u16, rows: u16) -> *mut c_void;
    fn cascade_vt_free(terminal: *mut c_void);
    fn cascade_vt_feed(terminal: *mut c_void, bytes: *const u8, len: usize);
    fn cascade_vt_feed_with_responses(
        terminal: *mut c_void,
        bytes: *const u8,
        len: usize,
        write: Writer,
    ...
    fn cascade_vt_set_appearance(terminal: *mut c_void, values: *const u32, notify: bool, write: Writer, userdata: *mut c_void) -> i32;
    fn cascade_vt_resize(terminal: *mut c_void, cols: u16, rows: u16) -> i32;
    fn cascade_vt_resize_geometry(
        terminal: *mut c_void,
        cols: u16,
        rows: u16,
        cell_width: u32,
    ...
    fn cascade_vt_snapshot(terminal: *mut c_void, write: Writer, userdata: *mut c_void) -> i32;
    fn cascade_vt_restore(bytes: *const u8, len: usize) -> *mut c_void;
    fn cascade_vt_format(terminal: *mut c_void, write: Writer, userdata: *mut c_void) -> i32;
    fn cascade_vt_cursor(terminal: *mut c_void, x: *mut u16, y: *mut u16) -> i32;
    fn cascade_vt_mode(terminal: *mut c_void, number: u16, ansi: bool, value: *mut bool) -> i32;
}
...
impl Terminal {
    pub fn new(cols: u16, rows: u16) -> Result<Self, Error> {
        ...
        NonNull::new(unsafe { cascade_vt_new(cols, rows) })
            .map(Self)
            .ok_or(Error("Could not create Ghostty terminal"))
    }
    pub fn feed(&mut self, bytes: &[u8]) {
        unsafe { cascade_vt_feed(self.0.as_ptr(), bytes.as_ptr(), bytes.len()) }
    }
    ...
    pub fn set_appearance(&mut self, appearance: &Appearance, notify: bool) -> Result<Vec<u8>, Error> {
        ...
        check(unsafe { cascade_vt_set_appearance(self.0.as_ptr(), appearance.0.as_ptr(), notify,
    ...
    fn collect_responses(
        ...
        check(
            unsafe {
                cascade_vt_feed_with_responses(
                    self.0.as_ptr(),
                    bytes.as_ptr(),
                    bytes.len(),
                    writer,
    ...
    pub fn resize(&mut self, cols: u16, rows: u16) -> Result<(), Error> {
        check(
            unsafe { cascade_vt_resize(self.0.as_ptr(), cols, rows) },
    ...
    pub fn restore(bytes: &[u8]) -> Result<Self, Error> {
        ...
        NonNull::new(unsafe { cascade_vt_restore(bytes.as_ptr(), bytes.len()) })
            .map(Self)
    ...
    pub fn formatted(&mut self) -> Result<Vec<u8>, Error> {
        self.collect(cascade_vt_format, "Could not format terminal state")
    }
    pub fn cursor(&mut self) -> Result<(u16, u16), Error> {
        let (mut x, mut y) = (0, 0);
        check(
            unsafe { cascade_vt_cursor(self.0.as_ptr(), &mut x, &mut y) },
    ...
    pub fn mode(&mut self, number: u16, ansi: bool) -> Result<bool, Error> {
        let mut value = false;
        check(
            unsafe { cascade_vt_mode(self.0.as_ptr(), number, ansi, &mut value) },
...
impl Drop for Terminal {
    fn drop(&mut self) {
        unsafe { cascade_vt_free(self.0.as_ptr()) }
    }
}
