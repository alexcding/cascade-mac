...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut terminal = cascade_vt::Terminal::new(args[0].parse()?, args[1].parse()?)?;
    if args.len() == 4 {
        terminal.resize_geometry(cascade_vt::Geometry {
    ...
    std::io::stdin()
        .take(cascade_vt::SNAPSHOT_LIMIT as u64 + 1)
        .read_to_end(&mut input)?;
    if input.len() > cascade_vt::SNAPSHOT_LIMIT {
