...
changes, and reviews is the hard part. Cascade gives every task its own git worktree,
...
- **Real terminals.** Sessions run in Ghostty-powered terminals owned by a separate
  ...
  Quitting Cascade stops them. Voice input works in terminals and agents once you allow
...
- **Open in your tools.** Open any worktree in Xcode, VS Code, Cursor, Windsurf, Zed,
  ...
  Fork, Tower, Sourcetree, GitHub Desktop, or your own Git client command. For Xcode, Cascade resolves Swift packages in the
...
a prompt and a goal Cascade checks when the agent finishes its turn. Steps can use
...
- Launch at login, CPU and memory use, and an activity log. The sidebar bell shows
  today's events.
- `cascade://app/…` links open the overview, the terminal, a project section, or a
  session.
- Cascade updates itself. Use **Check for Updates…** to check now.
...
Cascade works with the tools you already use. Connect only the ones you need:
...
Cascade has no account and no hosted backend. Your data stays on your Mac, in
`~/Library/Application Support/Cascade`; see [data recovery](docs/DATA-RECOVERY.md) for
...
Cascade is a SwiftUI and AppKit app with a Rust backend linked into the same process.
...
Cascade is built around real development work, and you don't need to know Swift or
...
- **Bring a workflow.** Show how you use agents, worktrees, or Jira, and the friction
  you'd like Cascade to remove. Open an issue before a large change so we can agree on
  the approach.
- **Help people find it.** Star the repository, share it with a teammate, or post a
  walkthrough of a task you finished with Cascade.
...
Cascade is free and open source under the [MIT License](LICENSE): anyone can use,
modify, and share it, including commercially.

Cascade builds on [Ghostty](https://github.com/ghostty-org/ghostty),
